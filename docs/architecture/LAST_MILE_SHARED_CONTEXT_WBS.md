...
Phase 1〜10 はマージ済み。中核仕様 + 取得手段 + Example が出揃い、汎用パッケージとして「導入可能な状態」に到達した。残るは Phase 11 (実プロジェクト導入) と Phase 12 (npm 公開準備)。
...
| #6 | 6 | MCP Server (8 tools) | 2026-05-17 11:16 |
...
| `@last-mile-context/cli` | ✅ | `lastmile collect / init / validate / mask / doctor` (commander ベース) |
| `@last-mile-context/mcp-server` | ✅ | MCP SDK 1.29 の `McpServer.registerTool` で 8 tools 公開 |
| `@last-mile-context/playwright-adapter` | ✅ | Adapter / accessibility / trace / actions / testGenerator |
...
- `examples/nextjs-app-router/` (Next.js 15 + React 19 の最小実用例)
